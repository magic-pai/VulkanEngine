...
void main() {
    ...
    float alphaWeight = clamp(materialAlpha * 8.0 + 0.01, 0.01, 1.0);
    float weight = depthWeight * alphaWeight;
    vec3 premultipliedColor = max(litColor, vec3(0.0)) * materialAlpha;

    outAccum = vec4(premultipliedColor * weight, materialAlpha * weight);
    outRevealage = clamp(1.0 - materialAlpha, 0.0, 1.0);
}
