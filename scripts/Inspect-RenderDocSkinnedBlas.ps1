...
param(
    ...
    [string]$CapturePath,
    [string]$OutputPath = "tmp\renderdoc_skinned_blas_inspection.json",
    [switch]$Strict
)
...
$pythonScript = Join-Path $PSScriptRoot "Inspect-RenderDocSkinnedBlas.py"
...
try {
    ...
    while (!(Test-Path -LiteralPath $OutputPath)) {
        if ($process.HasExited) {
            throw "qrenderdoc exited without a skinned BLAS inspection report"
        }
        if ((Get-Date) -ge $deadline) {
            Stop-Process -Id $process.Id -Force -ErrorAction SilentlyContinue
            throw "RenderDoc skinned BLAS inspection timed out"
        }
...
Write-Host (
    "RenderDoc skinned BLAS: {0} pass / {1} fail" -f
        $report.passCount,
        $report.failCount
)
Write-Host "Report: $OutputPath"
if ($report.status -ne "complete") {
    throw "RenderDoc skinned BLAS inspection failed: $($report.error)"
}
